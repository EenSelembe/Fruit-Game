...
function extractFirstColorFromGradient(grad){
  const m = String(grad||'').match(/(#(?:[0-9a-fA-F]{3,8}))|rgba?\([^)]*\)/);
  return m ? m[0] : '#000';
}
...
function buildNicknameStyle(data){
  ...
  let borderColorCanvas = data.borderColor || '#000';
  let extraBorder = '';
  if (data.borderGradient) {
    ...
    borderColorCanvas = extractFirstColorFromGradient(data.borderGradient);
  }
