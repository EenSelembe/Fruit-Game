...
const Game = (() => {
  ...
  // Profil style (dari firebase-boot via controller)
  let profileName='USER', profileTextColor='#fff', profileBorderColor='#000', profileBg=null, profileHasGradient=false;

  // ===== Input =====
  const keys = {};
  const pointer = { x:0, y:0, down:false };
  let joy, knob, joyState={ax:0,ay:0,active:false}, boostHold=false;
  ...
  function bindInputs(){
    ...
    if(joy && knob){
      ...
      function handleJoy(e,type){
        ...
        const pt = e.touches?.[0] ?? e; const dx=pt.clientX-cx, dy=pt.clientY-cy;
        const rad=r.width/2, mag=Math.hypot(dx,dy), cl=mag>rad?rad:mag;
  ...
  // ===== Geometry =====
  const bodyRadius = s => 4 + 2*Math.sqrt(Math.max(0,s.length-3));
  const BASE_SEG_SPACE = 6;
  const segSpace = s => Math.max(BASE_SEG_SPACE, bodyRadius(s)*0.9);

  // ===== Snake =====
  function createSnake(colors,x,y,isBot=false,len=3){
    const s={ id:Math.random().toString(36).slice(2), colors, x,y,
      dir:Math.random()*Math.PI*2-Math.PI, speedBase:120,speedMax:220,v:0,
      boost:false, energy:1, length:len, baseLen:len, fruitProgress:0,
      path:[], _pathAcc:0, alive:true, isBot, aiTarget:{x:Math.random()*WORLD.w,y:Math.random()*WORLD.h} };
    s.path.unshift({x:s.x,y:s.y}); return s;
  }
  const needForNext = s => 10 + Math.max(0,(s.length - s.baseLen))*2;
    ...
    switch(f.kind){
      ...
      case 'starfruit': ctx.fillStyle='#e9ff70'; ctx.beginPath();
        for(let i=0;i<5;i++){ const a=-Math.PI/2 + i*2*Math.PI/5;
          const x1=Math.cos(a)*11,y1=Math.sin(a)*11, x2=Math.cos(a+Math.PI/5)*5,y2=Math.sin(a+Math.PI/5)*5;
  ...
  function chaikinSmooth(pts,it=2){
    let out=pts.slice();
    for(let k=0;k<it;k++){
      const res=[out[0]];
      for(let i=0;i<out.length-1;i++){
        const p=out[i], q=out[i+1];
        res.push({x:p.x*0.75+q.x*0.25,y:p.y*0.75+q.y*0.25},{x:p.x*0.25+q.x*0.75,y:p.y*0.25+q.y*0.75});
      }
  ...
  function strokeStripedPath(pts,strokeWidth,colors,outlineWidth=0){
    if(pts.length<2) return; const smTailHead=chaikinSmooth(pts,2);
    ...
    if(cols.length<=1){
      ...
      const smHeadTail=smTailHead.slice().reverse(); const stripeLen=Math.max(18,strokeWidth*1.4);
      let acc=0, colorIdx=0, segStartIdx=0;
      function strokeSeg(a,b,col){ if(b<=a) return;
        ctx.beginPath(); ctx.moveTo(smHeadTail[a].x,smHeadTail[a].y); for(let j=a+1;j<=b;j++) ctx.lineTo(smHeadTail[j].x,smHeadTail[j].y);
  ...
  function drawSnake(sn){
    ...
    const nscr=worldToScreen(sn.x,sn.y), padX=34, padY=16*camera.zoom;
    ctx.save(); ctx.fillStyle='rgba(0,0,0,.35)'; ctx.fillRect(nscr.x-padX, nscr.y-22*camera.zoom, padX*2, padY);
    ctx.strokeStyle=profileBorderColor||'#000'; ctx.lineWidth=1.5; ctx.strokeRect(nscr.x-padX, nscr.y-22*camera.zoom, padX*2, padY);
    ctx.font=`${12*camera.zoom}px system-ui,Segoe UI`; ctx.textAlign='center'; ctx.textBaseline='bottom';
    ctx.fillStyle=profileTextColor||'#fff'; ctx.fillText(profileName||'USER', nscr.x, nscr.y-10*camera.zoom);
    ctx.restore();
  }
  ...
  function updateSnake(s,dt){
    ...
    if(s===player){
      ...
      targetAngle=Math.atan2(dy,dx)+(Math.random()*0.36-0.18); s.boost=Math.random()<0.012;
    }
    ...
    for(let i=foods.length-1;i>=0;i--){
      ...
      if(dx*dx+dy*dy<eatR*eatR){ foods.splice(i,1); s.fruitProgress+=1; if(s.fruitProgress>= (10+Math.max(0,(s.length-s.baseLen))*2) ){ s.fruitProgress=0; s.length+=1; } }
    }
  ...
  function killSnake(s){
    ...
    if(s.isBot){
      setTimeout(()=>{ const idx=snakes.indexOf(s); if(idx>=0) snakes.splice(idx,1);
        snakes.push(createSnake(['#79a7ff'], Math.random()*WORLD.w, Math.random()*WORLD.h, true, 3+Math.floor(Math.random()*8))); }, 700);
    }else{
  ...
  function updateRankPanel(){
    ...
    rankRowsEl.innerHTML = top.map((s,i)=>{
      const me=(s===player)?' me':''; return `<div class="rrow${me}"><div class="title">${i+1}. User</div><div class="sub">Len ${s.length}</div></div>`;
    }).join('');
  }
  ...
  function startGame(colors,startLen){
    snakes.length=0; foods.length=0; ensureFood(); spawnBots(BOT_NUM);
    player = createSnake((colors&&colors.length)?colors:['#58ff9b'],
      Math.random()*WORLD.w*0.6+WORLD.w*0.2,
      Math.random()*WORLD.h*0.6+WORLD.h*0.2, false, startLen||3);
    snakes.push(player); camera.x=player.x; camera.y=player.y; camera.zoom=1;
    lastColors=(colors&&colors.length)?colors:['#58ff9b']; lastStartLen=startLen||3;
    if(elLen) elLen.textContent=player.length; if(elUsers) elUsers.textContent=snakes.filter(s=>s.alive).length; updateRankPanel();
  }
  function quickReset(){
    startGame(lastColors,lastStartLen);
    const toast=document.getElementById('toast'); if(toast){ toast.textContent='Reset!'; toast.style.display='block';
  ...
  function loop(now){
    const frameDt=Math.min(0.1,(now-last)/1000); last=now; stepPhysics(frameDt);
    if(player){
    ...
    ctx.clearRect(0,0,vw,vh); drawGrid(); drawFood(); for(const s of snakes) drawSnake(s);
  ...
  function applyProfileStyle(style){
    if(!style) return;
    profileName=style.name||'USER'; profileTextColor=style.color||'#fff';
    ...
    }else{ profileBorderColor=style.borderColor||'#000'; }
    profileBg=style.bgGradient||style.bgColor||null; profileHasGradient=!!style.bgGradient;
  }

  return { init, start:startGame, quickReset, applyProfileStyle };
})();
