...
const Game = (() => {
  ...
  // Profil pemain aktif (untuk nameplate warna border/teks)
  let myName = 'USER';
    ...
    switch (f.kind) {
      ...
      case 'orange':
        ...
        for (let a=0;a<6;a++){ ctx.beginPath(); ctx.moveTo(0,0);
          ctx.lineTo(Math.cos(a*Math.PI/3)*9, Math.sin(a*Math.PI/3)*9); ctx.stroke(); } break;
      case 'grape':
      ...
      case 'strawberry':
        ...
        for (let i=-4;i<=4;i+=4){ for (let j=-2;j<=6;j+=4){ ctx.beginPath(); ctx.arc(i,j,1,0,Math.PI*2); ctx.fill(); } } break;
      case 'lemon':
      ...
      case 'blueberry':
        ...
        ctx.beginPath(); ctx.moveTo(-3,-1); ctx.lineTo(0,-4); ctx.lineTo(3,-1); ctx.lineTo(0,2); ctx.closePath(); ctx.fill(); break;
      case 'starfruit':
        ...
        ctx.strokeStyle='rgba(0,0,0,.15)'; ctx.lineWidth=1; ctx.stroke(); break;
    }
    ctx.restore();
  }
  ...
  function updateSnake(s, dt) {
    ...
    if (s === player) {
      ...
      // AI sederhana untuk bot (termasuk fallback offline)
  ...
  function killSnake(s) {
    ...
    // Remote player: hapus saja (akan muncul lagi saat ada update)
  ...
  function updateRankPanel() {
    ...
    // Rank berdasarkan semua ular yang "alive" (player + remote + bot),
    // tapi hanya PLAYER & REMOTE yang akan benar2 publish ke Firestore (bot tidak).
  ...
  /* ===== Spawn offline users as bots (opsional) ===== */
  ...
  function startGame(colors, startLen) {
    ...
    const uid = window.App?.profile?.id || window.Firebase?.auth?.currentUser?.uid || null;
  ...
  function netUpsert(uid, state) {
    ...
    if (!s) {
      ...
      s = createSnake(cols,
        ...
        false,
        typeof state?.length === 'number' ? state.length : (typeof state?.len === 'number' ? state.len : 3),
        name,
        uid
      );
      s.isRemote = true;
    ...
    const L = (typeof state.length === 'number') ? state.length
            : (typeof state.len === 'number' ? state.len : undefined);
    if (typeof L === 'number') s.length = Math.max(1, Math.floor(L));
    ...
    if (state.alive === false) s.alive = false;
  ...
  function netRemove(uid) {
    ...
    if (!s) return;
    // Jadikan bot agar tetap ada di peta saat user offline
    s.isRemote = false;
    s.isBot = true;
