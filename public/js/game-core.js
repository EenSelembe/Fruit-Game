...
const Game = (() => {
  ...
  function killSnake(s) {
    ...
    // Jika remote player mati, biarkan tetap dihapus.
    if (s.isRemote) {
      removeSnake(s);
      return;
    }
  ...
  function spawnOfflineAsBots(maxCount = 12) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const s = createSnake(baseCols, p.x, p.y, true, 3 + Math.floor(Math.random() * 8), u.name, uid);
      if (u.isAdmin) {
