...
//   Game.quickReset()
//   Game.applyProfileStyle(style)   // untuk pemain aktif
...
//   Game.getPlayerState()           // dipakai net-sync publish
...
const Game = (() => {
  ...
  function updateSnake(s, dt) {
    ...
    // tabrakan (hanya ke tubuh musuh, bukan tubuh sendiri)
  ...
  function startGame(colors, startLen) {
    ...
    // nameplate style utk player sesuai profil
    const sMe = createSnake(cols, startX, startY, false, startLen || 3, myName, uid, myTextColor, myBorderColor);
    ...
    // spawn offline users sebagai bot (nama & style asli)
  ...
  function netUpsert(uid, state) {
    ...
    if (!s) {
      // ambil style nameplate dari Presence
  ...
  // profil pemain aktif → style nameplate player
